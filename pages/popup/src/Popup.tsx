...
const Popup = () => {
  ...
  // 使用icon-128.png作为logo
  const logo = 'icon-128.png';
  ...
  return (
    <div className={cn('App', isLight ? 'bg-slate-50' : 'bg-gray-800')}>
      <header className={cn('App-header', isLight ? 'text-gray-900' : 'text-gray-100')}>
        ...
        <button
          className={cn(
            'mt-4 rounded px-6 py-2 font-bold shadow transition-transform hover:scale-105',
            isLight ? 'bg-pink-200 text-black hover:bg-pink-300' : 'bg-pink-700 text-white hover:bg-pink-600',
          )}
